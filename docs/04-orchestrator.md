...
- **Die Tatsachen stehen im Hauptbuch** (`model_calls`, seit `e5f6a7b8c9d0`): eine Zeile je Modellaufruf mit Anbieter, Modell, Zweck, Tokens, Kosten und einem Zeitstempel aus `now()`. Geschrieben wird an genau **einer** Stelle — im Model Gateway, dem einzigen Weg zu einem Sprachmodell und der Stelle, an der die Kosten ohnehin errechnet werden. Ein Strukturtest hält fest, dass jeder Aufruf einen Abrechnungskontext mitgibt; `runs.usage` ist damit die **abgeleitete** Sicht, und ein Test rechnet sie gegen das Hauptbuch nach. Damit fällt auch die alte Schieflage: Kosten zählen zum Tag ihres **Aufrufs**, nicht zum Tag des Laufbeginns.
