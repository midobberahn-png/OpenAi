...
> Stand: Commit `644e9de`, fortgeschrieben, seit ein Modell die
> Werkzeugargumente formuliert und den Plan zu Ende führt.
...
| **Werkzeugwahl durch ein Modell** | Bislang wählt der **Plan** das Werkzeug und das Modell nur die Argumente — oder gar nichts, beim Antwortschritt. Sobald `ModelLoop` einen Endpunkt bekommt, wählt das Modell aus der Schnittmenge: eine andere und größere Fläche. | Offen. `AgentRuntime` berechnet das Angebot je Zugriff neu und `ModelLoop` führt nichts aus; geprüft ist beides nur ohne Endpunkt. Gemessen ist immerhin, dass ein Angebot ohne Werkzeuge auch keinen Werkzeugaufruf zurückbekommt. |
...
| **Werkzeugergebnisse im Modellkontext** | Beide Modellquellen geben heute nur Schritt-Zusammenfassungen (Pfad, Bytezahl), nicht die gelesenen Daten. Sobald sie die Daten bekommen, steht Fremdinhalt im Prompt — und die Herkunftsmarkierung ist dann nicht mehr Vorsichtsmaßnahme, sondern die ganze Absicherung. | **Der nächste Engpass.** Beim Durchstich gemessen: „lies X und fasse zusammen" läuft durch und antwortet „ich kenne den Inhalt nicht". Die Mechanik (`Message.is_untrusted`, `ModelGateway._kontaminiert`) steht und entscheidet bislang über wenig. Vorher zu klären: wie viel Inhalt, wo gekappt, und ob Fremdinhalt im Prompt ausgezeichnet wird. |
| **Modellantwort an einen Menschen** | Der abschließende `llm`-Schritt erzeugt Text, der einem Menschen gezeigt wird. Stammt er aus einem kontaminierten Lauf, kann er eine untergeschobene Anweisung an *ihn* tragen („laut Notiz sollst du überweisen"). | **Vom Taint-Tracking nicht zu schließen** — es sperrt Werkzeuge, und hier ist keines beteiligt. Was der Kern leistet: Der Lauf bleibt `tainted` über den Abschluss hinweg, und `GET /runs/{id}` zeigt es. Eine Oberfläche, die das nicht kennzeichnet, lässt die Lücke offen (`test_http_runs.py::TestAntwortschritt`). |
